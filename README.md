...
docker compose down
```
> Если порт 5432 уже занят локальным PostgreSQL — либо останови свой локальный сервис Postgres, либо поменяй проброс порта в `docker-compose.yml` с `"5432:5432"` на, например, `"5433:5432"` (второе число трогать не нужно — это порт внутри контейнера).
